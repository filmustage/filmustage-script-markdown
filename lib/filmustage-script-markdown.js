	...
	convert (pref) {
		...
		if (editor) {
			...
			if (pref !== "location") {
				...
				var textArticles = textSs.replace(/^a\s|^an\s|^the\s/gi, "");
			}
			else {
			...
			if (pref !== "delete-tag") {
				selection.insertText(prefix + textUntag + suffix, {select: true});
			}
			else {
				selection.insertText(symbolsBegin + textUntag + symbolsEnd, {select: true});
			}
		}
	},

	...
	addCSS() {
		...
		for (index = 1; index < mTags.length + 1; ++index) {
			...
			touchBar[index - 1] = {
				"type": "button",
				"name": "f" + index + "-button",
				"label": "" + bLabel.substring(0, 10) + "",
				"command": "filmustage:tag-f" + index + "",
				"color": "" + rgbaToHex(atom.config.get("filmustage-script-markdown.f" + index + "Color")) + "",
				"icon": ""
			};
		}
	...
	convert(pref) {
		...
		if (editor) {
			...
			if (editor.getLastSelection().isEmpty()) {
				...
				console.log(cursor.getCurrentWordPrefix());
